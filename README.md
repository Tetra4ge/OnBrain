...
Extracted entities are mapped into a unified knowledge graph (Neo4j) with relationships across document types — linking a specific pump in a P&ID to its failure history and its maintenance manual. A vector store (ChromaDB) enables semantic search, while Firestore tracks all document metadata.
...
| **Knowledge** | ChromaDB, Neo4j, Firestore | Document metadata, graph, and semantic retrieval |
...
| **Document DB** | Firestore | Document metadata, sync state, chat history, and RCA reports |
...
| **Phase 2 — Data & Schema** | Demo documents & schema lock | Firestore, Neo4j, ChromaDB schemas | [phase-2-data-schema.md](./phases/phase-2-data-schema.md) |
| **Phase 3 — Ingestion** | OCR, extraction, normalizer | Full ingestion pipeline | [phase-3-ingestion.md](./phases/phase-3-ingestion.md) |
| **Phase 4 — Knowledge Layer** | Graph & vector store population | Neo4j + ChromaDB + Firestore sync | [phase-4-knowledge.md](./phases/phase-4-knowledge.md) |
